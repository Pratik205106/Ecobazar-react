...
const ProductModal = ({ product, onClose }: ProductModalProps) => {
  ...
  const rating = typeof product.star === "string" ? parseFloat(product.star) : product.star ?? 0;
  const discountPercent = product.fixRate
    ? Math.round(((parseFloat(product.fixRate.replace("$", "")) - product.price) / parseFloat(product.fixRate.replace("$", ""))) * 100)
    : null;
  ...
  const renderStars = (rating: number) => {
    ...
    return (
      <div className="flex items-center gap-1">
        {[...Array(fullStars)].map((_, i) => <FaStar key={i} className="text-[#FF8A00]" />)}
        {halfStar && <FaStarHalfAlt className="text-[#FF8A00]" />}
        {[...Array(emptyStars)].map((_, i) => <FaRegStar key={i} className="text-[#FF8A00]" />)}
      </div>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-md p-6 w-full max-w-[1320px] h-[636px] relative flex flex-col md:flex-row gap-8 overflow-y-auto">

        {/* Close Button */}
        <button onClick={onClose} className="absolute top-4 right-4 text-2xl text-gray-700">
          <IoClose />
        </button>
        ...
        <div className="flex items-center space-x-4">
          <h2 className="text-[36px] font-semibold text-[#1A1A1A]">{product.name}</h2>
   ...
   <h1 className="mx-auto"> In Stock</h1>
          ...
          <div className="flex items-center gap-2 text-sm mt-3">
            ...
            <span className="text-[#666666] text-[14px]">{product.reviewCount ?? "4"} Review</span>
            <hr className="bg-[#B3B3B3] border-4 rounded-full"/>
             <div className="flex items-center gap-4 text-sm">
            <span className="text-[#333333] font-medium">SKU: 
             <span className="text-[#666666]"> {product.sku ?? "000-000"}
              </span>
              </span>
          ...
          <div className="flex items-center gap-4 mt-5">
            {product.fixRate && <span className="line-through text-gray-400 text-lg">{product.fixRate}</span>}
            <span className="text-[#2C742F] text-[24px] font-medium">${product.price.toFixed(2)}</span>
            {discountPercent && (
